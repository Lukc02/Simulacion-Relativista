...
aB0  = 0.0_wp
  ...
  ! A <- B (obs = A, src = B)
  !call accel_LL_step_AB( x_obs=rA, v_obs=vA,                 &
                       !  r_src_tr=rB0, v_src_tr=vB0, a_src_tr=aB0, q_src=q2, &
                       !  q_obs=q1, m_obs=mA, tau0_obs=tau0A,  &
                        ! a_obs=aA0 )
...
!tau0B = tau0_LL(q2, mB)
 ...
 ! call accel_LL_step_AB( x_obs=rB, v_obs=vB,                 &
                       !  r_src_tr=rA0, v_src_tr=vA0, a_src_tr=aA0, q_src=q1, &
                        ! q_obs=q2, m_obs=mB, tau0_obs=tau0B,  &
                       !  a_obs=aB0 )
...
!tau0A = tau0_LL(q1, mA)
!call accel_LL_step_AB( x_obs=rA, v_obs=vA,                 &
                        ! r_src_tr=rB1, v_src_tr=vB1, a_src_tr=aB1, q_src=q2, &
                       !  q_obs=q1, m_obs=mA, tau0_obs=tau0A,  &
                         !a_obs=aA1 )
...
!tau0B = tau0_LL(q2, mB)
!call accel_LL_step_AB( x_obs=rB, v_obs=vB,                 &
                        ! r_src_tr=rA1, v_src_tr=vA1, a_src_tr=aA1, q_src=q1, &
                        ! q_obs=q2, m_obs=mB, tau0_obs=tau0B,  &
                       !  a_obs=aB1 )
...
! A <- B (obs = A, src = B)
!tau0A = tau0_LL(q1, mA)
 !call accel_LL_step_AB( x_obs=rA - chi*dtA1*vA, v_obs=vA - chi*dtA1*aA1,                 &
                         !r_src_tr=rBtp, v_src_tr=vBtp, a_src_tr=aB1, q_src=q2, &
                         !q_obs=q1, m_obs=mA, tau0_obs=tau0A,  &
                         !a_obs=aA_stage )


!call accel_LL_step_AB( x_obs=rB - chi*dtB1*vB, v_obs=vB - chi*dtB1*aB1,                 &
                       !  r_src_tr=rAtp, v_src_tr=vAtp, a_src_tr=aA1, q_src=q1, &
                       ! q_obs=q2, m_obs=mB, tau0_obs=tau0B,  &
                        ! a_obs=aB_stage )
...
!call accel_LL_step_AB( x_obs=rA, v_obs=vA,                 &
                        ! r_src_tr=rB2_at, v_src_tr=vB2_at, a_src_tr=aB1, q_src=q2, &
                        ! q_obs=q1, m_obs=mA, tau0_obs=tau0A,  &
                       !  a_obs=aA2 )
...
!call accel_LL_step_AB( x_obs=rB, v_obs=vB,                 &
                        ! r_src_tr=rA2_at, v_src_tr=vA2_at, a_src_tr=aA1, q_src=q1, &
                        ! q_obs=q2, m_obs=mB, tau0_obs=tau0B,  &
                        ! a_obs=aB2 )
